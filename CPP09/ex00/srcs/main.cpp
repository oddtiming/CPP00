...
void print_usage(string executableName, string message = "Error.") {
  std::cerr << message << " Usage: " << executableName
            << " [path_to_input_file]\n\n"
            << "Input format:\n"
            << "      Date | Value \n"
            << "YYYY-MM-DD | [0-1000]\n"
...
int daysSinceNewYear(uint year, uint month, uint day) {
  int totalDays = 0;
  int daysInFebruary = isLeapYear(year) ? 29 : 28;
  uint daysInMonth[] = {31, daysInFebruary, 31, 30, 31, 30, 31, 31, 30, 31, 30,
                        31};

  // Check if year is valid
  if (year < MIN_YEAR || year > MAX_YEAR) {
    std::cerr << "Error: year " << year << " should be between " << MIN_YEAR
              << " and " << MAX_YEAR << std::endl;
    return -1;
  }

  // Check if that month has at least ${day} days in it
  if (day > daysInMonth[month]) {
    std::cerr << "Error: the month " << year << "-" << month + 1 << " only has "
              << daysInMonth[month] << " days in it." << std::endl;
    return -1;
  }
...
void testStrToUnixTime() {
  struct Test {
    std::string input;
    bool expectValid;
  };

  Test tests[] = {{"2001-02-21", true},
                  {"2012-12-12", true},
                  {"2022-01-10", true},
                  {"202-00-10", false},
...
float getClosestPrecedingValue(map<uint, float> &db, uint date) {
  ...
  std::cout << "For " << date << ", found closest value to be at " << it->first
            << " for a value of " << it->second << std::endl;
  return it->first;
}

void calculatePrices(map<uint, float> &db, string inputStr) {
  size_t eol = 0;
  size_t start = 0;
  string row;
