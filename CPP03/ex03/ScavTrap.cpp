...
void		ScavTrap::takeDamage( uint damage ) {
	...
	if ( this->_guardMode && _updateLuckFactor() ) {
		...
		std::cout << "\t" << GUARD_EMOJI  << " Guard mode gives ScavTrap " << *this << " an unexpected protection !" << "\n"
	...
	std::cout << "\nScavTrap " << *this << " takes " << damage << " damage. Fuckin ouch." << std::endl;
...
void		ScavTrap::beRepaired( uint amount ) {
	...
	if ( this->_guardMode && _updateLuckFactor() ) {
		...
		std::cout << "\t" << GUARD_EMOJI  << " Guard mode gives ScavTrap " << *this << " an unexpected boost !" << "\n\t"
...
bool		ScavTrap::_updateEnergy( ) {
	...
	if ( this->_guardMode && _updateLuckFactor() ) {
		...
		std::cout << "\t" << GUARD_EMOJI << " Guard mode gives ScavTrap " << *this << " unexpected energy!"
	...
	if (!this->_energyPts) {
		...
		std::cout << "No can do! ScavTrap " << *this << " has no more energy points" << std::endl;
		return false;
	}
